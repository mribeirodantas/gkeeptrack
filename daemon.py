...
"""
    ...
    This was based on what Andrew Cuddon published on
...
class DaemonClass:
    ...
    def before_stop(self):
        """
            Override this method if you need to do something before the
            daemon/service STOPS
        """
        pass
    ...
    def stop(self):
        """
            ...
            Stops the original daemon and creates a new daemon, enabling
            configuration changes to be reloaded
        """
    ...
    def stop_linux_daemon(self):
        """
            Terminates the daemon running in another process (by issuing a
            SIGTERM - terminate signal). This will be issued only by a second
            instance such as from the command line, e.g.:
                mydaemon.py stop
            The daemon being terminated will trap the signal and exit
            gracefully.
        """
        ...
        if self.mode == 'stop':
            # Note: we do not want to exit when we are doing a restart
            sys.exit(0)

    def before_restart(self):
        """
            Override this method if you need to do something before the
            daemon/service RESTARTS
            Restart notes:
                before_stop() and before_start() are also triggered when
                restarting the daemon
                before_stop() will be triggered on the daemonised processes
                before_start() will be triggered on the current process
                (which will become the new dameon)
        """
        pass
    ...
    def restart(self):
        ...
        if self.platform == "Linux":
            ...
            # Stops the original daemon process by issuing a SIGTERM
            self.stop_linux_daemon()
            # Starts a new daemon in the current process
            self.start_linux_daemon()
        else:
            pass
        # Passes processing control to the run method
        self.run()
    ...
    def status(self):
        ...
        if self.platform == "Linux":
            message = ""
            # Get the pid of the running daemon (not the current process)
            # from the pidfile
            if os.path.exists(self.pidfile):
                try:
                    pf = file(self.pidfile,'r')
            ...
            sys.stdout.write("Daemon mode not currently supported on %s OS," +
                             " so running non-demonised...\n" % self.platform)
            return None

    def run(self):
        """
            This is where your processing occurs
            This must be a continuous look or the daemon will exit once
            processing has finished. The loop finished when an interrupt
            signal is received and 'alive' is set to False
        ...
        i = 1
        while self.alive:
            # Do something. In this case just write a line of text and wait
            # for 1 second, then do it all over again
            sys.stdout.write('%d: Daemon/Service is running with a' +
                             'process id(pid)=%s\n' % (i, self.pid))
            i+=1
            try:
                time.sleep(1)
            except:
                # traps and ignores any error if an interrupt is
                # received while sleeping/waiting
                pass
    ...
    def on_exit(self):
        """
            Override this method for any additional clean up processing just
    ...
    def signal_handler(self, signum, frame):
        ...
        """
        signals = {15: 'SIGTERM (TERMinate SIGnal)',
                   2:  'SIGINT (INTerupt (Ctrl-C) SIGnal)'}
...
if __name__ == '__main__':
    # Testing Only
    # The Daemon class should be subclassed and the run(), tidyup()
    # and on_interrupt() methods overridden
    app = None
